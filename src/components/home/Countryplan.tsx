...
export default function CountryplanTabs() {
  ...
  const handleAuthSuccess = () => {
    ...
    if (selectedPlan) {
      navigation(
        `/country/checkout?plan=${selectedPlan.id}&country=${selectedPlan?.country?.id}`,
      );
    }
  };
          ...
          <div className="pt-0 md:pt-5">
            ...
            {active === "popular" && (
              <div role="tabpanel" aria-label="Popular plans">
                <div
                  className={`mt-10 !pb-0 ${featured.length === 0 ? "flex justify-center" : "flex flex-wrap gap-x-6 gap-y-6 justify-center"}`}
                >
                  {featured.length === 0 ? (
                    <div className="flex justify-center items-center py-12">
                      <svg
  ...
  bg-white border-gray-200 rounded-xl hover:bg-green-50 hover:border-[#3BC852] border-2 transition duration-300 mb-0 "
          ...
          <div className="flex flex-col items-center gap-4  mt-6  md:mt-15">
            <p className="text-white  text-center text-lg sm:text-xl ">
              Explore eSIMs and stay connected anywhere in the world.
            </p>
            <button
              onClick={(e) => {
                e.preventDefault();
              ...
              className="mt-3 bg-[#fff] text-center text-[#133365] max-[425px]:!text-[14px] max-[330px]:!text-[12px]  hover:text-white px-6 py-2.5 rounded-full hover:bg-[#3BC852] transition text-[16px]"
            >
              Explore Esims Now
            </button>
          </div>
